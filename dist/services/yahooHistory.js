...
exports.fetchHistoricalCatchup = fetchHistoricalCatchup;
...
async function fetchHistoricalCatchup() {
    ...
    `);
        ...
        const upsertToDb = async (result, FinInstrmId) => {
            if (!result || result.length === 0)
                return;
            // Filter out rows with null close/open (incomplete trading days or live intraday artifacts)
            const cleanResult = result.filter((row) => row.close !== null && row.open !== null);
            if (cleanResult.length === 0)
                return;
            const values = cleanResult.map((row) => [
                FinInstrmId,
            ...
            try {
                ...
                if (fallbackSymbol) {
                    try {
                        ...
                        fs_1.default.appendFileSync(logFilePath, `${new Date().toISOString()} - ${FinInstrmId}, Primary: ${primarySymbol}, Fallback: ${fallbackSymbol}, Error: ${fallbackErr.message}\n`);
                    }
                }
                else {
                    console.error(`No fallback available for ${primarySymbol}. Logging to failed_fetches.log`);
                    fs_1.default.appendFileSync(logFilePath, `${new Date().toISOString()} - ${FinInstrmId}, Primary: ${primarySymbol}, No Fallback, Error: ${err.message}\n`);
                }
            }
        }
    }

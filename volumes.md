Currently the project uses the price history api to get the volume I want you to create a new worker for fetching the volumes of stocks part but it will also require creating a new table one is bse_volume and other is nse_volume what we want for the new worker that is npm run start:volume is to download bhavcopy
...
For NSE: nsearchives.nseindia.com/products/content/sec_bhavdata_full_11092026.csv in this also change the date
